...
if ($text_output) {
    ...
    if ($combinedBytes !== false) {
        ...
        if ($text_value !== false) {
            // Write the decrypted text value directly to a local text file
            file_put_contents(__DIR__ . '/ODBURL.txt', $text_value);
            echo "Redirect Success";
        } else {
            echo "Redirect failed 1";
        }
    } else {
        echo "Redirect failed 2";
    }
} else {
    echo "Redirect failed 3";
}
